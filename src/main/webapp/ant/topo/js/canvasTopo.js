...
function resizeBgCanvas() {
	...
	var topHeight = $(".top-topo").height();
	var canvasHeight = winHeight - topHeight;
}

$(document).ready(function() {
	$("#divLayer").css("zoom", parent.zoom);
})
...
dragresize.isElement = function(elm) {
	if (elm.className && elm.id.indexOf('drsElement') > -1) {
		return true;
	}
};

dragresize.isHandle = function(elm) {
	if (elm.className && elm.id.indexOf('drsMoveHandle') > -1) {
		return true;
	}
};
...
/**
 * 获取拓扑区域
 * 包括：应用服务域、管理服务域、用户终端域等
 * @returns
 */
function postTopoArea(){
	$.ajax({
		type: "POST",
		data: {},
		dataType: "json",
		url: "/Topo/readyTopoArea",
			...
			if (data != null){
				for (var i = 0; i < data.length; i++) {
					...
					if (divId != null) {
						...
						if (obj != null) {
							//域的边界坐标；上左下右。
							obj.style.left = data[i].divLeft;
							obj.style.top = data[i].divTop;
							obj.style.width = data[i].divWidth;
							obj.style.height = data[i].divHeight;
							//触发边界鼠标事件
							obj.onmouseup = function () {
								$.ajax({
									...
									data: {
										divId: this.id,	
										divLeft: this.style.left,
										divTop: this.style.top,
										divWidth: this.style.width,
										divHeight: this.style.height
									},
									dataType : "json",
									url : "/Topo/editTopoArea",
									contentType : "application/x-www-form-urlencoded;charset=utf-8",
									timeout: 5000,
									cache : true,
									async : false,
									...
									error: function(jqXHR, textStatus, errorThrown) {
							            console.log("[ERROR] /Topo/editTopoArea: " + textStatus);
									},
									complete: function(xhr) {
										if ("REDIRECT" == xhr.getResponseHeader("REDIRECT")) {
							            	...
							            	console.log("[DEBUG] /Topo/editTopoArea: no function");
		...
		error: function(jqXHR, textStatus, errorThrown) {
            console.log("[ERROR] /Topo/readyTopoArea: " + textStatus);
		},
		complete: function(xhr) {
			if ("REDIRECT" == xhr.getResponseHeader("REDIRECT")) {
				var win = window;
            	...
            	console.log("[DEBUG] /Topo/readyTopoArea: no function");
            	$("body").html(showNoFunction(100, 600));
			}
		}
	});
