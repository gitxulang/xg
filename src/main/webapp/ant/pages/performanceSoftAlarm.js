...
function loadList(json) {
    ...
    for (var i in json.list) {
        ...
        if (json.list[i].nmsAlarmSoft.dstatus == "0") {
            ...
            action = '<a href="#" onclick="alarmDeal(' + softId + ',' + typeId + ',' + json.list[i].nmsAlarmSoft.id + ',' + json.list[i].nmsAlarmSoft.dstatus + ')"><i class="my-material-icons">mode_edit</i></a>';
            ...
            action = '<a href="#" onclick="alarmDeal(' + softId + ',' + typeId + ',' + json.list[i].nmsAlarmSoft.id + ',' + json.list[i].nmsAlarmSoft.dstatus + ')"><i class="my-material-icons">search</i></a>';
            action += '<a href="#" onclick="alarmDelete(' + json.list[i].nmsAlarmSoft.id + ')"><i class="my-material-icons">delete</i></a>';
        } else {
        ...
        $("#tableBodyID").append(
            ...
            '<td>' + json.list[i].nmsAlarmSoft.stime + '</td>' +
			'<td>' + json.list[i].nmsAlarmSoft.atime + '</td>' +
			'<td>' + json.list[i].nmsAlarmSoft.acount + '</td>' +
            '<td>' + dealTime + '</td>' +
            '<td>' + status + '</td>' +
...
function alarmDeal(softId, typeId, alarmId, status) {
    if (status == 0) {
        var url = "alarmSoftPreDeal.html?fromType=performance&softId=" + softId + "&typeId=" + typeId + "&alarmId=" + alarmId;
        window.location = url;
    } else if (status == 1) {
        var url = "alarmSoftDeal.html?fromType=performance&softId=" + softId + "&typeId=" + typeId + "&alarmId=" + alarmId;
        window.location = url;
    } else if (status == 2) {
        var url = "alarmSoftDeal.html?fromType=performance&softId=" + softId + "&typeId=" + typeId + "&alarmId=" + alarmId;
        window.location = url;
