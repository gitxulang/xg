...
$(document).ready(
    function () {
        $("body").append(
            ...
            + '<div class="orderHttpsProtocol_count" style="display:none;">0</div>'
            + '<div class="orderProtocols_count" style="display:none;">0</div>'
            + '<div class="orderListenPorts_count" style="display:none;">0</div>'
            + '<div class="orderMaxConnNum_count" style="display:none;">0</div>'
            + '<div class="orderMaxHreadNum_count" style="display:none;">0</div>'
...
function setOrderKey(obj) {
    ...
    if (order == "orderStartTime") {
    ...
    } else if (order == "orderHttpsProtocol") {
        $(".orderHttpsProtocol_count").html(parseInt($(".orderHttpsProtocol_count").html()) + 1);
        orderKey = "https_protocol";
        orderValue = parseInt($(".orderHttpsProtocol_count").html()) % 2;

        $(".current_orderkey").html(orderKey);
        $(".current_ordervalue").html(orderValue);
    } else if (order == "orderProtocols") {
        $(".orderProtocols_count").html(parseInt($(".orderProtocols_count").html()) + 1);
        orderKey = "protocols";
        orderValue = parseInt($(".orderProtocols_count").html()) % 2;

        $(".current_orderkey").html(orderKey);
        $(".current_ordervalue").html(orderValue);
    } else if (order == "orderListenPorts") {
        $(".orderListenPorts_count").html(parseInt($(".orderListenPorts_count").html()) + 1);
        orderKey = "listen_ports";
        orderValue = parseInt($(".orderListenPorts_count").html()) % 2;
        ...
        $(".current_orderkey").html(orderKey);
        $(".current_ordervalue").html(orderValue);
    } else if (order == "orderMaxHreadNum") {
        $(".orderMaxHreadNum_count").html(parseInt($(".orderMaxHreadNum_count").html()) + 1);
        orderKey = "max_hread_num";
        orderValue = parseInt($(".orderMaxHreadNum_count").html()) % 2;
...
function query(dataJson) {
    ...
    $.ajax({
        ...
        data: dataJson,
        dataType: "json",
        url: "/MiddlewareConfig/list/date",
...
function loadList(json) {
    ...
    for (var i in json.list) {
        ...
        if (json.list[i].httpsProtocol == 1) {
            httpsProtocol = "已启用";
        } else {
            httpsProtocol = "未启用";
        }
        ...
        $("#tableBodyID").append(
            ...
            '<td>' + httpsProtocol + '</td>' +
            '<td>' + json.list[i].protocols + '</td>' +
            '<td>' + json.list[i].listenPorts + '</td>' +
            '<td>' + json.list[i].maxConnNum + '</td>' +
            '<td>' + json.list[i].maxHreadNum + '</td>' +
            '<td>' + format(json.list[i].itime) + '</td>' +
            '<td class="center"><a href="#" value="'+ json.list[i].id + '" onclick="openInstance(this)"><i class="my-material-icons">search</i></a></td>' +
            '</tr>'
        );
    }
    $("#page").html(json.page);
...
function openInstance(obj) {
    var id = $(obj).attr("value");
    var url = "middlewareInstanceInfo.html?id=" + id;
    open_dialog(url, "实例列表", 800, 800);
}
...
function exportTable() {
    var url = "/MiddlewareConfig/list/date/ExportExcel?"
    var start_time = $("#start_time").val();
    var end_time = $("#end_time").val();
    var id = $("#assetId").val();
