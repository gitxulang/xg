	...
	function() {
		$("body").append(
			'<div class="current_orderkey" style="display:none;"></div>'
					...
					+ '<div class="orderPing_count" style="display:none;">0</div>'
					+ '<div class="orderRtt_count" style="display:none;">0</div>'
...
function setOrderKey(obj) {
	...
	var orderValue = 0;
	if (order == "orderPing") {
		$(".orderPing_count").html(parseInt($(".orderPing_count").html()) + 1);
		orderKey = "ping_rate";
		orderValue = parseInt($(".orderPing_count").html()) % 2;

		$(".current_orderkey").html(orderKey);
		$(".current_ordervalue").html(orderValue);
	} else if (order == "orderRtt") {
		$(".orderRtt_count").html(parseInt($(".orderRtt_count").html()) + 1);
		orderKey = "ping_rtt";
		orderValue = parseInt($(".orderRtt_count").html()) % 2;
	...
	for ( var i in json.list) {
		...
		$("#tableBodyID").append(
				'<tr class="odd gradeX">' + '<td class="center">' + n + '</td>' + '<td>'
						+ pingRate + '</td>' + '<td>' + json.list[i].pingRtt
...
function exportTable() {
	var url = "/PingInfo/list/date/ExportExcel?"
	var start_time = $("#start_time").val();
	var end_time = $("#end_time").val();
	var id = $("#assetId").val();
