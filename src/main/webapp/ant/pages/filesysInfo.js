					...
					$("body")
							.append(
									'<div class="current_orderkey" style="display:none;"></div>'
											...
											+ '<div class="orderFree_count" style="display:none;">0</div>'
											+ '<div class="orderInodeTotal_count" style="display:none;">0</div>'
											+ '<div class="orderInodeFree_count" style="display:none;">0</div>'
...
function setOrderKey(obj) {
	...
	if (order == "orderName") {
		...
		$(".orderTotal_count")
				.html(parseInt($(".orderTotal_count").html()) + 1);
		orderKey = "part_total";
		orderValue = parseInt($(".orderTotal_count").html()) % 2;
		$(".current_orderkey").html(orderKey);
		$(".current_ordervalue").html(orderValue);
	} else if (order == "orderFree") {
		$(".orderFree_count").html(parseInt($(".orderFree_count").html()) + 1);
		orderKey = "part_free";
		orderValue = parseInt($(".orderFree_count").html()) % 2;
		$(".current_orderkey").html(orderKey);
		$(".current_ordervalue").html(orderValue);
	} else if (order == "orderInodeTotal") {
		$(".orderInodeTotal_count").html(
				parseInt($(".orderInodeTotal_count").html()) + 1);
		orderKey = "part_inode_num";
		orderValue = parseInt($(".orderInodeTotal_count").html()) % 2;
		$(".current_orderkey").html(orderKey);
		$(".current_ordervalue").html(orderValue);
	} else if (order == "orderInodeFree") {
		$(".orderInodeFree_count").html(
				parseInt($(".orderInodeFree_count").html()) + 1);
		orderKey = "part_inode_free";
		orderValue = parseInt($(".orderInodeFree_count").html()) % 2;
		$(".current_orderkey").html(orderKey);
		$(".current_ordervalue").html(orderValue);
	...
	for ( var i in json.list) {
		...
		$("#tableBodyID").append(
				'<tr class="odd gradeX">' + '<td class="center">' + n + '</td>' + '<td>'
						+ json.list[i].fileSys + '</td>' + '<td>'
						+ json.list[i].partTotal + '</td>' + '<td>'
						+ json.list[i].partFree + '</td>' + '<td>'
						+ json.list[i].partInodeNum + '</td>' + '<td>'
						+ json.list[i].partInodeFree + '</td>' + '<td>'
...
function exportTable() {
	var url = "/FilesysInfo/list/date/ExportExcel?"
	var start_time = $("#start_time").val();
	var end_time = $("#end_time").val();
	var id = $("#assetId").val();
	url += "assetId=" + id + "&";
