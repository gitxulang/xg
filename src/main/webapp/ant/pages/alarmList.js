...
function alarmDeal(alarmId, status) {
	...
	if (status == 0) {
		openDialog('alarmPreDeal.html?alarmId=' + alarmId, "告警信息处理", 800, 600);
	} else if (status == 1) {
		openDialog('alarmDeal.html?alarmId=' + alarmId, "告警信息处理", 800, 600)
		
	} else if (status == 2) {
		openDialog('alarmDeal.html?alarmId=' + alarmId, "告警信息查看", 800, 600)
