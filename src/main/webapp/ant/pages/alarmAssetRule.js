...
$(document).ready(function() {
	$("body").append(
		...
		'<div class="orderAssetName_count" style="display:none;">0</div>'+
...
function setOrderKey(obj) {
	...
	} else if (order == "orderAssetName") {
		$(".orderAssetName_count").html(parseInt($(".orderAssetName_count").html())+1);
		orderKey = "a.a_name";
		orderValue = parseInt($(".orderAssetName_count").html())%2;
...
function query(dataJson) {
	...
	$.ajax({
		...
		data: dataJson,
		dataType: "json",
		url: "/RuleAsset/findNmsRuleAsset",
		...
		error: function(jqXHR, textStatus, errorThrown) {
            console.log("[ERROR] /RuleAsset/findNmsRuleAsset: " + textStatus);
		},
		complete: function(xhr) {
			if ("REDIRECT" == xhr.getResponseHeader("REDIRECT")) {
            	...
            	console.log("[DEBUG] /RuleAsset/findNmsRuleAsset: no function");
...
function updateAlarmRule(id) {
	openDialog("alarmAssetRuleUpdate.html?id=" + id, "阈值规则修改", 800, 600)
}
...
var enableAlarm = function(id, renable) {
	...
	$.ajax({
		type: 'POST',
		dataType: "json",
		url: "/RuleAsset/updateNmsRuleAssetMonitorById",
		data: dataJson,
		contentType: 'application/x-www-form-urlencoded;charset=utf-8',
		timeout: 5000,
		cache: true,
		...
		error: function(jqXHR, textStatus, errorThrown) {
            console.log("[ERROR] /RuleAsset/updateNmsRuleAssetMonitorById: " + textStatus);
		},
		complete: function(xhr) {
			if ("REDIRECT" == xhr.getResponseHeader("REDIRECT")) {
				var win = window;
            	...
            	console.log("[DEBUG] /RuleAsset/updateNmsRuleAssetById: no function");
            	$("#page-inner").html(showNoFunction(100, 600));
			}
		}
	});
