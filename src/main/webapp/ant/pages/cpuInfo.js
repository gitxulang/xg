					...
					$("body")
							.append(
									'<div class="current_orderkey" style="display:none;"></div>'
											...
											+ '<div class="orderIndex_count" style="display:none;">0</div>'
											+ '<div class="orderFreq_count" style="display:none;">0</div>'
											+ '<div class="orderRate_count" style="display:none;">0</div>'
...
function setOrderKey(obj) {
	...
	} else if (order == "orderIndex") {
		$(".orderIndex_count")
				.html(parseInt($(".orderIndex_count").html()) + 1);
		orderKey = "cpu_cores";
		orderValue = parseInt($(".orderIndex_count").html()) % 2;
		$(".current_orderkey").html(orderKey);
		$(".current_ordervalue").html(orderValue);
	} else if (order == "orderFreq") {
		$(".orderFreq_count").html(parseInt($(".orderFreq_count").html()) + 1);
		orderKey = "cpu_freq";
		orderValue = parseInt($(".orderFreq_count").html()) % 2;
		$(".current_orderkey").html(orderKey);
		$(".current_ordervalue").html(orderValue);
	} else if (order == "orderRate") {
		$(".orderRate_count").html(parseInt($(".orderRate_count").html()) + 1);
		orderKey = "cpu_rate";
		orderValue = parseInt($(".orderRate_count").html()) % 2;
		$(".current_orderkey").html(orderKey);
		$(".current_ordervalue").html(orderValue);
...
function loadList(json) {
	...
	for ( var i in json.list) {
		...
		var cpuName = json.list[i].cpuName == '' ? '--' : json.list[i].cpuName;
		var cpuFreq = json.list[i].cpuFreq == '' ? '--' : json.list[i].cpuFreq;
		if (cpuFreq == 0) {
			cpuFreq = "--";
		}
		var cpuRate = (json.list[i].cpuRate + "") == null ? '--' : (parseFloat(json.list[i].cpuRate)).toFixed(2);

		$("#tableBodyID").append(
				'<tr class="odd gradeX">' + '<td class="center">' + n + '</td>' + '<td>'
						+ cpuName + '</td>' + '<td>' + json.list[i].cpuCores
						+ '</td>' + '<td>' + cpuFreq + '</td>' + '<td>'
						+ cpuRate + '</td>' + '<td>'
...
function exportTable() {
	var url = "/CpuInfo/list/date/ExportExcel?"
	var start_time = $("#start_time").val();
	var end_time = $("#end_time").val();
	var id = $("#assetId").val();
	url += "assetId=" + id + "&";
