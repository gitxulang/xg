...
$(document).ready(function() {
	$("body").append(
		...
		'<div class="orderRealname_count" style="display:none;">0</div>' +
		...
		'<div class="orderEmail_count" style="display:none;">0</div>' +
		'<div class="orderPhone_count" style="display:none;">0</div>' +
...
function setOrderKey(obj) {
	...
	if (order == "orderName") {
	...
	} else if (order == "orderRealname") {
		$(".orderRealname_count").html(parseInt($(".orderRealname_count").html()) + 1);
		orderKey = "realname";
		orderValue = parseInt($(".orderRealname_count").html()) % 2;
		...
		$(".current_orderkey").html(orderKey);
		$(".current_ordervalue").html(orderValue);			
	} else if (order == "orderEmail") {
		$(".orderEmail_count").html(parseInt($(".orderEmail_count").html()) + 1);
		orderKey = "email";
		orderValue = parseInt($(".orderEmail_count").html()) % 2;
		
		$(".current_orderkey").html(orderKey);
		$(".current_ordervalue").html(orderValue);			
	} else if (order == "orderPhone") {
		$(".orderPhone_count").html(parseInt($(".orderPhone_count").html()) + 1);
		orderKey = "phone";
		orderValue = parseInt($(".orderPhone_count").html()) % 2;
	...
	for (var i in json.list) {
		...
		$("#tableBodyID").append(
			'<tr class="odd gradeX">'
					...
					+ '<td>' + htmlEscape(json.list[i].username) + '</td>'
					+ '<td>' + htmlEscape(json.list[i].realname) + '</td>'
					+ '<td>' + htmlEscape(sex) + '</td>'
					+ '<td>' + htmlEscape(json.list[i].email) + '</td>'
					+ '<td>' + htmlEscape(json.list[i].phone) + '</td>'
					+ '<td>' + htmlEscape(json.list[i].nmsRole.role) + '</td>'
...
function doAdd() {
	var url = "systemUserAdd.html";
	jumpPage(url);
//	window.location.href = url
}

function doUpdate(id) {
	var url = "systemUserUpdate.html?id=" + id;
	jumpPage(url);
//	window.location.href = url;
}
