...
$(document).ready(
    function () {
        $("body").append(
            ...
            + '<div class="orderSlowSql_count" style="display:none;">0</div>'
            + '<div class="orderExecTime_count" style="display:none;">0</div>'
...
function setOrderKey(obj) {
    ...
    var orderKey = "";
    var orderValue = 0;
    if (order == "orderSlowSql") {
        $(".orderSlowSql_count").html(parseInt($(".orderSlowSql_count").html()) + 1);
        orderKey = "slow_sql";
        orderValue = parseInt($(".orderSlowSql_count").html()) % 2;

        $(".current_orderkey").html(orderKey);
        $(".current_ordervalue").html(orderValue);
    } else if (order == "orderExecTime") {
        $(".orderExecTime_count").html(parseInt($(".orderExecTime_count").html()) + 1);
        orderKey = "exec_time";
        orderValue = parseInt($(".orderExecTime_count").html()) % 2;
...
function loadList(json) {
    ...
    for (var i in json.list) {
        ...
        $("#tableBodyID").append(
            ...
            '<td>' + json.list[i].slowSql + '</td>' +
            '<td>' + json.list[i].execTime + '</td>' +
...
function exportTable() {
    var url = "/DatabaseSql/list/date/ExportExcel?"
    var start_time = $("#start_time").val();
    var end_time = $("#end_time").val();
    var id = $("#statusId").val();
