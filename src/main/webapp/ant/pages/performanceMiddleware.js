...
function loadDevice() {
    ...
    $.ajax({
        ...
        success: function (json) {
            ...
            if (json != null && json.configInstances != null) {
                loadList(json.configInstances);
            } else {
           //     $("#tableBodyID").append(showNoData(100, 517));
            ...
            if (json == null) {
                ...
                if (json.numPerSecond == null) {
                    ...
                    dash_board_num("current_rps_rate", "每秒请求数", "请求数(个)", json.numPerSecond.toFixed(2) + "", 5000, 8);
                }
            }
        },
		...
		complete: function(xhr) {
			if ("REDIRECT" == xhr.getResponseHeader("REDIRECT")) {
	            ...
	            $("#tableBodyID").append(showNoFunction(100, 517));
...
function loadTable(json) {
    ...
    innerTable += '<tbody>';
    innerTable += '<tr>';
    innerTable += '<td class="my-table-col" width="30%">软件/中间件名称</td>';
    innerTable += '<td class="success">' + (json.softName==null?'--':json.softName) + '/' + (json.name==null?'--':json.name) + '</td>';
    innerTable += '</tr>';

    innerTable += '<tr>';
    innerTable += '<td class="my-table-col">中间件/JDK版本</td>';
    innerTable += '<td class="success">' + (json.version==null?'--':json.version) + '/' + (json.jdkVersion==null?'--':json.jdkVersion) + '</td>';
    innerTable += '</tr>';

    innerTable += '<tr>';
    innerTable += '<td class="my-table-col">告警状态</td>';
    if (json.statusCode == null || json.statusCode == 1) {
    ...
    innerTable += '<tr>';
    innerTable += '<td class="my-table-col">中间件/JVM最大连接数</td>';
    innerTable += '<td class="success">' + (json.maxConnNum==null?'--':json.maxConnNum) + '/' + (json.maxHreadNum==null?'--':json.maxHreadNum) + '</td>';
    innerTable += '</tr>';

    var httpsProtocol = '';
    if (json.httpsProtocol == null) {
        httpsProtocol = "--"
    } else if (json.httpsProtocol == 0) {
        httpsProtocol = "未启用"
    } else {
        httpsProtocol = "已启用"
    }
    innerTable += '<tr>';
    innerTable += '<td class="my-table-col">是否启用HTTPS</td>';
    innerTable += '<td class="success">' + httpsProtocol + '</td>';
    innerTable += '</tr>';


    innerTable += '<tr>';
    innerTable += '<td class="my-table-col">最大内存量（KB）</td>';
    innerTable += '<td class="success">' + (json.memTotal==null?'--':json.memTotal) + '</td>';
    innerTable += '</tr>';

    innerTable += '<tr>';
    innerTable += '<td class="my-table-col">已用内存量（KB）</td>';
    innerTable += '<td class="success">' + (json.memUsed==null?'--':json.memUsed) + '</td>';
    innerTable += '</tr>';

    innerTable += '<tr>';
    innerTable += '<td class="my-table-col">堆内存量（KB）</td>';
    innerTable += '<td class="success">' + (json.heapTotal==null?'--':json.heapTotal) + '</td>';
    innerTable += '</tr>';

    innerTable += '<tr>';
    innerTable += '<td class="my-table-col">已用堆内存量（KB）</td>';
    innerTable += '<td class="success">' + (json.heapUsed==null?'--':json.heapUsed) + '</td>';
    innerTable += '</tr>';

    innerTable += '<tr>';
    innerTable += '<td class="my-table-col">非堆内存量（KB）</td>';
    innerTable += '<td class="success">' + (json.nonHeapTotal==null?'--':json.nonHeapTotal) + '</td>';
    innerTable += '</tr>';

    innerTable += '<tr>';
    innerTable += '<td class="my-table-col">已用非堆内存量（KB）</td>';
    innerTable += '<td class="success">' + (json.nonHeapUsed==null?'--':json.nonHeapUsed) + '</td>';
    innerTable += '</tr>';
...
function dash_board_num(show_id, show_name, show_unit, value, maxValue, splitNumber) {
    if (value == null || value == "") {
        document.getElementById(show_id).innerHTML = showNoData(document.getElementById(show_id).offsetWidth, document.getElementById(show_id).offsetHeight);
        return;
    }
