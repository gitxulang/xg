...
function query(dataJson) {
	...
	$.ajax({
		...
		data: dataJson,
		dataType: "json",
		url: "/alarm/soft/list/page/condition",
		...
		error: function(jqXHR, textStatus, errorThrown) {
            console.log("[ERROR] /alarm/soft/list/page/condition: " + textStatus);
		},
		complete: function(xhr) {
			if ("REDIRECT" == xhr.getResponseHeader("REDIRECT")) {
            	...
            	console.log("[DEBUG] /alarm/soft/list/page/condition: no function");
...
function loadList(json){
	...
	for (var i in json.list) {
		...
		if (json.list[i].nmsAlarmSoft.alevel == "1") {
			level = '<font color="green">低风险</font>';
		}
		
		if (json.list[i].nmsAlarmSoft.alevel == "2") {
			level = '<font color="orange">中风险</font>';
		}
		
		if (json.list[i].nmsAlarmSoft.alevel == "3") {
			level = '<font color="red">高风险</font>';
		}
		
		if (json.list[i].nmsAlarmSoft.dstatus == "0") {
			status = '<font color="red">待处理</font>';
			action = '<a href="#" onclick="alarmDeal(' + json.list[i].nmsAlarmSoft.id + ',' + json.list[i].nmsAlarmSoft.dstatus + ')"><i class="my-material-icons">mode_edit</i></a>';
			action += '<a href="#" onclick="alarmDelete(' + json.list[i].nmsAlarmSoft.id + ')"><i class="my-material-icons">delete</i></a>';
		} else if(json.list[i].nmsAlarmSoft.dstatus == "1") {
			status = '<font color="yellow">处理中</font>';
			action = '<a href="#" onclick="alarmDeal(' + json.list[i].nmsAlarmSoft.id + ',' + json.list[i].nmsAlarmSoft.dstatus + ')"><i class="my-material-icons">mode_edit</i></a>';
			action += '<a href="#" onclick="alarmDelete(' + json.list[i].nmsAlarmSoft.id + ')"><i class="my-material-icons">delete</i></a>';
		} else if (json.list[i].nmsAlarmSoft.dstatus == "2") {
			status = '<font color="green">已处理</font>';
			action = '<a href="#" onclick="alarmDeal(' + json.list[i].nmsAlarmSoft.id + ',' + json.list[i].nmsAlarmSoft.dstatus + ')"><i class="my-material-icons">search</i></a>';
			action += '<a href="#" onclick="alarmDelete(' + json.list[i].nmsAlarmSoft.id + ')"><i class="my-material-icons">delete</i></a>';
		} else {
			status = '<font color="green">未知</font>';
		}
		
		var showContent = json.list[i].nmsAlarmSoft.acontent;
		if (showContent != null) {
			showContent = showContent.substring(0, 80);
		}
		
		var dealTime = '';
		dealTime = json.list[i].nmsAlarmSoft.dtime;
		if (dealTime == '') {
			dealTime = '--';
		}
		...
		$("#tableBodyID").append(
			'<tr class="odd gradeX">' +
				...
				'<td>' + htmlEscape(json.list[i].nmsAlarmSoft.nmsSoft.aip) + '</td>' +
				'<td>' + htmlEscape(json.list[i].nmsAlarmSoft.nmsSoft.aport) + '</td>' +
				'<td>' + level + '</td>' +
				'<td>' + htmlEscape(json.list[i].nmsAlarmSoft.acontent) + '</td>' +
				'<td>' + json.list[i].nmsAlarmSoft.atime + '</td>' +
				'<td>' + dealTime + '</td>'+
		 		'<td>' + status + '</td>'+
...
function alarmDeal(alarmId, status) {
	if (status == 0) {
		var url = "alarmSoftPreDeal.html?alarmId=" + alarmId;
		window.location = url;
	} else if (status == 1) {
		var url = "alarmSoftDeal.html?alarmId=" + alarmId;
		window.location = url;
	} else if (status == 2) {
		var url = "alarmSoftDeal.html?alarmId=" + alarmId;
		window.location = url;
...
function alarmDelete(id) {
	...
	$.ajax({
		type: 'POST',
		dataType: "json",
		url: "/alarm/soft/deleteAlarmById",
		data: dataJson,
		contentType: 'application/x-www-form-urlencoded;charset=utf-8',
		timeout: 5000,
		cache: true,
		...
		error: function(jqXHR, textStatus, errorThrown) {
            console.log("[ERROR] /alarm/soft/deleteAlarmById: " + textStatus);
		},
		complete: function(xhr) {
			if ("REDIRECT" == xhr.getResponseHeader("REDIRECT")) {
				var win = window;
            	...
            	console.log("[DEBUG] /alarm/soft/deleteAlarmById: no function");
            	$("#page-inner").html(showNoFunction(100, 600));
			}
		}
	});
...
function exportTable() {
	var url = "/alarm/list/alarm/soft/condition/exportExcel?";
	var ip = $("#ip").val();
	var port = $("#port").val();
	var alarm_level = $("#alarm_level").val();
	var alarm_status = $("#alarm_status").val();
