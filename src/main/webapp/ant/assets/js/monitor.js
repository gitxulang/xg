function drawLayer02Label(canvasObj,text,textBeginX,lineEndX){
	...
	ctx.fillStyle = colorValue;
	ctx.fillText(text,textBeginX,96);
}
...
function renderLegend(){
	drawLegend(COLOR.MACHINE.TYPE_A,25,'A类IP');
	drawLegend(COLOR.MACHINE.TYPE_B,50,'B类IP');
	drawLegend(COLOR.MACHINE.TYPE_C,75,'C类IP');
	drawLegend(COLOR.MACHINE.TYPE_D,100,'D类IP');
	drawLegend(COLOR.MACHINE.TYPE_E,125,'E类IP');
	drawLegend(COLOR.MACHINE.TYPE_F,150,'F类IP');
	drawLegend(COLOR.MACHINE.TYPE_G,175,'G类IP');
}

function drawLegend(pointColor,pointY,text){
	var ctx = $("#layer03_left_01 canvas").get(0).getContext("2d");
	ctx.beginPath();
	ctx.arc(20,pointY,6,0,2*Math.PI);//小圆圈位置
	ctx.fillStyle = pointColor;
	ctx.fill();
	ctx.fontSize='20px';
	ctx.fillStyle = '#000000';
	ctx.fillText(text,40,pointY+4);//字体位置
}
		...
		myChart.setOption(
					 {
						...
						legend: {
							...
							x : 'center',
							y : 'bottom',
							data:['A类IP','B类IP','C类IP','D类IP','E类IP','F类IP','G类IP']
						},
						toolbox: {
						},
						label:{
						...
						series : [
							{
								...
								center: ['50%', '60%'],
								data:[
									{value:7600, name:'A类IP'},
									{value:6600, name:'B类IP'},
									{value:15600, name:'C类IP'},
									{value:5700, name:'D类IP'},
									{value:4600, name:'E类IP'},
									{value:4600, name:'F类IP'},
									{value:3500, name:'G类IP'}
								],
								itemStyle: {
									emphasis: {
										shadowBlur: 10,
...
function renderLayer04Right(){
	...
	myChart.setOption({
			...
			legend: {
				...
				orient:'vertical',
				data:[
						{name:'在线已登记',icon:'circle'},
						{name:'在线未登记',icon:'circle'},
						{name:'在线非法占用',icon:'circle'}
					]
			},
			grid: {
				left: '3%',
