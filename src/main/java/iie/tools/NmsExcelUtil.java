...
public class NmsExcelUtil {
	...
	public boolean exportExcel() {
		...
		// 设置表头内容
		Cell cell = null;
		if (namesOfHeader != null && namesOfHeader.size() > 0) {
			...
			for (int i = 0, imax = list.size(); i < imax; i++) {
				...
				for (int j = 0; j < namesOfField.size(); j++) {
					...
					try {
						...
						String v = (String) field.get(list.get(i));
	...
	public NmsExcelUtil(List<?> list, Class<?> clazz, String nameOfFile,
			String nameOfSheet, String nameOfTitle, List<String> namesOfHeader,
			List<String> namesOfField, HttpServletResponse response) {
		super();
		this.clazz = clazz;
