...
import iie.pojo.NmsMiddlewareBasic;
import iie.pojo.NmsSoft;
import iie.service.NmsAuditLogService;
import iie.service.NmsMiddlewareBasicService;
 ...
 */
@Controller
@RequestMapping("/MiddlewareBasic")
public class NmsMiddlewareBasicCtrl {
    @Autowired
    NmsMiddlewareBasicService mbService;
    ...
    @ResponseBody
    public PageBean<NmsMiddlewareBasic> listPageByDate(HttpServletRequest request, HttpSession session) throws Exception {
        ...
        PageBean<NmsMiddlewareBasic> page = mbService.getPageByDate(orderKey, orderValue, startDate, endDate, begin, offset, assetId);
    ...
    public boolean listPageByDateExportExcel(HttpServletRequest request, HttpServletResponse response, HttpSession session) throws UnsupportedEncodingException {
        ...
        try {
            ...
            String docName = "专用中间件基础信息报表" + asset.getAIp();
            List<Map> varList = new ArrayList<Map>();
            for (NmsMiddlewareBasic var : list) {
                Map<String, String> vpd = new HashMap<>();
                vpd.put("var1", var.getName());
                vpd.put("var2", var.getVersion());
                vpd.put("var3", var.getJdkVersion());
                vpd.put("var4", DateUtil.date2Str(new Date(var.getInstallTime())));
                vpd.put("var5", var.getProcessName());
            ...
            if (b) {
                auditLogService.add(
                        ((NmsAdmin) session.getAttribute("user")).getRealname(),
                        request.getRemoteAddr(), "导出专用中间件基础信息报表 | 软件名称：" + asset.getAName() + " | 软件IP及端口号：" + asset.getAIp() + ":" + asset.getAPort(), "监控管理", "导出文件", "成功");
            }
    ...
    @ResponseBody
    public NmsMiddlewareDetail assetMiddlewareInfoOverview(HttpServletRequest request) throws Exception {
        String softIdParam = request.getParameter("softId");
        Integer softId = -1;
        if (softIdParam != null && softIdParam.length() > 0) {
            softId = Integer.valueOf(softIdParam);
        }
        return mbService.assetMiddlewareInfoOverview(softId);
    }
}
