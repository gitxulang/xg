...
@Service("NmsMysqlInfoService")
public class NmsMysqlInfoService {
	@Autowired
	HibernateTemplate hibernateTemplate;

	@SuppressWarnings("unchecked")
	public NmsMysqlInfo getMysqlById(Integer id) {
		String hsql = "from NmsMysqlInfo nm where nm.nmsAsset.id = " + id;
		List<NmsMysqlInfo> list = (List<NmsMysqlInfo>) hibernateTemplate.find(hsql);
	...
	@SuppressWarnings("unchecked")
	public PageBean<NmsMysqlInfo> getPageByDate(String orderKey,
		...
		// 获取count总数
		String sqlCount = "select count(1) from nms_mysql_info as rt where rt.asset_id = '"
				+ assetId + "'";
		...
		// 获取list数据
		String sqlList = "select * from nms_mysql_info as rt where rt.asset_id = '"
				+ assetId + "'";
		if (startDate != null && endDate != null) {
		...
		// 序列化数据
		queryList.addEntity("NmsMysqlInfo", NmsMysqlInfo.class);
		List<NmsMysqlInfo> list = queryList.list();
		session.close();

		// 创建PageBean对象返回数据
		PageBean<NmsMysqlInfo> page = new PageBean<NmsMysqlInfo>();
	...
	@SuppressWarnings("unchecked")
	public List<NmsMysqlInfo> getPageByDateExportExcel(String orderKey,
	...
	public boolean deleteByAssetId(String assetId) {
		boolean res = true;
		String sql = "delete from nms_mysql_info where asset_id = " + assetId;
		Session session = hibernateTemplate.getSessionFactory().openSession();
		try {
